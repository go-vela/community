...
func main() {
	...
	app.Usage = "Vela utility used to migrate from v0.20.x to v0.21.x"
	...
	app.Flags = []cli.Flag{
		...
		&cli.BoolFlag{
			...
			Name:    "action.all",
			Usage:   "enables running all actions for v0.21.x",
		},
		&cli.BoolFlag{
			EnvVars: []string{"VELA_ALTER_TABLES", "ALTER_TABLES"},
			Name:    "alter.tables",
			Usage:   "enables altering the table configuration for v0.21.x",
		},
		&cli.BoolFlag{
			EnvVars: []string{"VELA_UPDATE_SCHEDULES", "UPDATE_SCHEDULES"},
			Name:    "update.schedules",
			Usage:   "enables updating existing schedules branch configuration for v0.21.x",
		},
