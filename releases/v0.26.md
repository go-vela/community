...
## v0.26.5 server + v0.27.3 UI

This patch release includes a couple bug fixes as well as a small enhancement to limit queue build up from restart requests.

**ADMINS**: Please note that this upgrade requires the addition of two database columns. While we normally avoid database changes for patch releases, the restart limit change was an exception due to its improvement in application stability combined with the significant gap between `v0.26` and the already released `v0.27`. 

- (server) fix(patch-v0.26.5): apply gatekeep build obj bug fix by @ecrupper in <https://github.com/go-vela/server/pull/1347>
- (server) fix(patch-v0.26.5): apply queue restart limits by @ecrupper in <https://github.com/go-vela/server/pull/1350>
- (server) fix(patch-v0.26.5): apply compiler WithUser middleware fix by @wass3rw3rk in <https://github.com/go-vela/server/pull/1351>
- (server) fix(patch-v0.26.5): apply updated dependencies by @ecrupper in <https://github.com/go-vela/server/pull/1354>
- (ui) fix: backport PR #858 (UI support for queue restart limit) by @wass3rw3rk in <https://github.com/go-vela/ui/pull/888>
