...
If you configured Vela to use a [custom webhook address](https://go-vela.github.io/docs/reference/installation/server#vela_scm_webhook_addr), it previously appened `/webhook` to the supplied address. It will now use the address as-is, so please ensure your configuration reflects this change.
