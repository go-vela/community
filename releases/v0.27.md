...
No changes in this release. The previous, immutable release was missing some release assets (API Spec, JSONSchema).

## v0.27.4

### 📦 go-vela/server

- fix(api): regen install token if expired for status update [#1375](https://github.com/go-vela/server/pull/1375) - thanks [@ecrupper](https://github.com/ecrupper)!
- chore(deps): update all non major deps for v0.27.4 patch release [#1376](https://github.com/go-vela/server/pull/1376) - thanks [@ecrupper](https://github.com/ecrupper)!

## v0.27.3

### 📦 go-vela/server

- fix(patch-v0.27.3): backport yaml expansion fix [#1362](https://github.com/go-vela/server/pull/1355) - thanks [@ecrupper](https://github.com/ecrupper)!

### 📦 go-vela/worker

- fix(secret): leverage outputs for secret container execution [#664](https://github.com/go-vela/worker/pull/664) - thanks [@ecrupper](https://github.com/ecrupper)!
...
The `VELA_SOURCE_CLIENT` and `VELA_SOURCE_SECRET` environment variables have been removed. If you were using these variables, please switch to using `VELA_SCM_CLIENT` and `VELA_SCM_SECRET`.
